...
{
    /// <summary>UI Toolkit implementation of <see cref="INXProgressBar"/>.</summary>
    public sealed class NXProgressBarUIToolkit : INXProgressBar
    {
        private readonly IUIValueCapability _value;

        public IUIElementHandle Handle { get; }

        public NXProgressBarUIToolkit(IUIElementHandle handle)
        {
            Handle = handle;
            _value = handle?.As<IUIValueCapability>();
            if (_value == null)
                Debug.LogWarning($"[NexUI] NXProgressBarUIToolkit: '{handle?.Id}' has no value capability.");
        }
