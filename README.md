...
- Unity **2022.3 LTS or newer**. Treat compilation, Unity Test Runner, Play Mode, and Player
  Build as separate release checks for each supported Unity version.
- Async APIs use `System.Threading.Tasks`. There is no third-party async dependency, so the
  package compiles as soon as it is imported.
